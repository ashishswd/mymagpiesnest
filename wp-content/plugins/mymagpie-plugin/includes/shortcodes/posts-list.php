...
if (!function_exists('posts_list_shortcode')) {
	...
	function posts_list_shortcode( $atts, $content = null, $shortcodename = '' ) {
		extract(shortcode_atts(array(
			...
			'order'        => '',
			'link'         => '',
			'link_text'    => __('Read more', MYMAGPIE_PLUGIN_DOMAIN),
			'tag'          => '',
			'tags'         => '',
			'custom_class' => ''
		), $atts));
			...
			$format = get_post_format();
				...
				$output .= '<article class="span12 post__holder">';
					...
					$output .= '<header class="post-header">';
						...
						$output .= '<div class="post_meta">';
							...
							if ($type!='' && $type!='post') {
								...
								if ( $terms && ! is_wp_error( $terms ) ) {
									$out = array();
									$output .= __('Posted in', MYMAGPIE_PLUGIN_DOMAIN).' ';
						...
						$output .= __('by ', MYMAGPIE_PLUGIN_DOMAIN).' ';
						...
						if( $cc == $num || $cc > 1 ) : $cc = $cc.' '.__('Comments', MYMAGPIE_PLUGIN_DOMAIN);
						else : $cc = $cc.' '.__('Comments', MYMAGPIE_PLUGIN_DOMAIN);
						endif;
						...
						$output .= '</div>';
						$output .= mymagpie_get_post_networks(array('post_id' => $post_id, 'display_title' => false, 'output_type' => 'return'));
					$output .= '</header>';
					...
					if ($tags == 'yes') {
						...
						if ($posttags) {
							$output .= '<footer class="post_footer">'.__('Tags', MYMAGPIE_PLUGIN_DOMAIN).": ";
