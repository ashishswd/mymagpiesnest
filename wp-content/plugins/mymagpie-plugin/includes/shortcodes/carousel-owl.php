...
if ( !function_exists('shortcode_carousel_owl') ) {
	function shortcode_carousel_owl( $atts, $content = null, $shortcodename = '' ) {
		wp_enqueue_script( 'owl-carousel', MYMAGPIE_PLUGIN_URL . 'lib/js/owl-carousel/owl.carousel.min.js', array('jquery'), '1.31', true );
				...
				while ( $carousel_query->have_posts() ) : $carousel_query->the_post();
					...
					$output .= '<div class="item ' . $format . ' item-list-'.$itemcounter.'">';
						...
						$output .= '<div class="desc">';
							...
							// post author
							$output .= $author ? '<em class="author">&nbsp;<span>' . __('by ', MYMAGPIE_PLUGIN_DOMAIN) . '</span>&nbsp;<a href="' . get_author_posts_url( get_the_author_meta( 'ID' ) ).'">' . get_the_author_meta( 'display_name' ) . '</a> </em>' : '';
							...
							if ( $more_text_single != '' ) {
								$output .= '<a href="' . get_permalink( $post_id ) . '" class="btn btn-primary" title="' . $post_title_attr . '">';
									$output .= __( $more_text_single, MYMAGPIE_PLUGIN_DOMAIN );
								$output .= '</a>';
							}
						$output .= '</div>';
					$output .= '</div>';
