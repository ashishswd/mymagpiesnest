...
function widgets_scripts($hook) {
	...
	// widget rules JS
	wp_register_script('widget-rules-js', MYMAGPIE_PLUGIN_URL.'admin/js/widget-rules.js', array('jquery'));
	wp_enqueue_script('widget-rules-js');
	// widget rules CSS
	wp_register_style('widget-rules-css', MYMAGPIE_PLUGIN_URL.'admin/css/widget-rules.css');
...
add_filter( 'widget_update_callback', 'mymagpie_widget_update', 10, 4);

// definition of the additional operation
function mymagpie_widget_update($instance, $new_instance, $old_instance, $widget) {
	$themename = 'mymagpie';
	clean_rules();
...
add_filter( 'dynamic_sidebar_params', 'mymagpie_dynamic_sidebar_params' );
function mymagpie_dynamic_sidebar_params( $params ) {
...
add_action( 'sidebar_admin_setup', 'mymagpie_add_widget_control');
function mymagpie_add_widget_control() {
	global $wp_registered_widgets; 
	...
	foreach ( $wp_registered_widgets as $id => $widget ) {
		if ( isset($wp_registered_widget_controls[$id]) ) {
			...
			$wp_registered_widget_controls[$id]['callback'] = 'mymagpie_widget_control';
		}
	}
}

// function to add the widget control 
function mymagpie_widget_control() {
	...
	echo '<p class="custom_class"><label for="' . $themename . '_widget_custom_class_'.$id.'">'.__('Custom CSS class', MYMAGPIE_PLUGIN_DOMAIN).': <input type="text" class="widefat" name="' . $themename . '_widget_custom_class_'.$id.'"  id="' . $themename . '_widget_custom_class_'.$id.'" value="'.$c_class.'" /></label></p>';
	echo '
	<a class="gk_widget_rules_btn button">'.__('Visibility', MYMAGPIE_PLUGIN_DOMAIN).'</a>
	<div class="gk_widget_rules_wrapper'.((isset($_COOKIE['gk_last_opened_widget_rules_wrap']) && $_COOKIE['gk_last_opened_widget_rules_wrap'] == 'gk_widget_rules_form_'.$id) ? ' active' : '').'" data-id="gk_widget_rules_form_'.$id.'">
		<p>
			<label for="' . $themename . '_widget_rules_'.$id.'">'.__('Visible at', MYMAGPIE_PLUGIN_DOMAIN).': </label>
			<select name="' . $themename . '_widget_rules_type_'.$id.'" id="' . $themename . '_widget_rules_type_'.$id.'" class="widefat gk_widget_rules_select">
				<option value="all"'.(($value_type != "include" && $value_type != 'exclude') ? " selected=\"selected\"":"").'>'.__('All pages', MYMAGPIE_PLUGIN_DOMAIN).'</option>
				<option value="exclude"'.(($value_type == "exclude") ? " selected=\"selected\"":"").'>'.__('All pages expecting', MYMAGPIE_PLUGIN_DOMAIN).':</option>
				<option value="include"'.(($value_type == "include") ? " selected=\"selected\"":"").'>'.__('No pages expecting', MYMAGPIE_PLUGIN_DOMAIN).':</option>
			</select>
		</p>
		<fieldset class="gk_widget_rules_form" id="gk_widget_rules_form_'.$unique_id.'" data-id="gk_widget_rules_form_'.$id.'">
			<legend>'.__('Select page to add', MYMAGPIE_PLUGIN_DOMAIN).'</legend>
			<select class="widefat gk_widget_rules_form_select">
				<option value="homepage">'.__('Homepage', MYMAGPIE_PLUGIN_DOMAIN).'</option>
				<option value="page:">'.__('Page', MYMAGPIE_PLUGIN_DOMAIN).'</option>
				<option value="post:">'.__('Post', MYMAGPIE_PLUGIN_DOMAIN).'</option>
				<option value="category:">'.__('Archive', MYMAGPIE_PLUGIN_DOMAIN).'</option>
				<option value="tag:">'.__('Tag', MYMAGPIE_PLUGIN_DOMAIN).'</option>
				<option value="archive">'.__('Archive', MYMAGPIE_PLUGIN_DOMAIN).'</option>
				<option value="author:">'.__('Author', MYMAGPIE_PLUGIN_DOMAIN).'</option>
				<option value="search">'.__('Search page', MYMAGPIE_PLUGIN_DOMAIN).'</option>
				<option value="page404">'.__('404 page', MYMAGPIE_PLUGIN_DOMAIN).'</option>
			</select>
			<p><label>'.__('Page ID/Title/slug', MYMAGPIE_PLUGIN_DOMAIN).':<input type="text" class="gk_widget_rules_form_input_page" /></label></p>
			<p><label>'.__('Post ID/Title/slug', MYMAGPIE_PLUGIN_DOMAIN).':<input type="text" class="gk_widget_rules_form_input_post" /></label></p>
			<p><label>'.__('Category ID/Name/slug', MYMAGPIE_PLUGIN_DOMAIN).':<input type="text" class="gk_widget_rules_form_input_category" /></label></p>
			<p><label>'.__('Tag ID/Name', MYMAGPIE_PLUGIN_DOMAIN).':<input type="text" class="gk_widget_rules_form_input_tag" /></label></p>
			<p><label>'.__('Author', MYMAGPIE_PLUGIN_DOMAIN).':<input type="text" class="gk_widget_rules_form_input_author" /></label></p>
			<p><button class="gk_widget_rules_btn button-secondary">'.__('Add page', MYMAGPIE_PLUGIN_DOMAIN).'</button></p>
			...
			<fieldset class="gk_widget_rules_pages">
				<legend>'.__('Selected pages', MYMAGPIE_PLUGIN_DOMAIN).'</legend>
				<span class="gk_widget_rules_nopages">'.__('No pages', MYMAGPIE_PLUGIN_DOMAIN).'</span>
				<div></div>
			</fieldset>
		</fieldset>
		<script type="text/javascript">gk_widget_control_init(\'#gk_widget_rules_form_'.$unique_id.'\');</script>';
		// create the list of suffixes
	mymagpie_widget_control_styles_list($params[0]['widget_id'], $id, $responsiveMode, $usersMode);
}

function mymagpie_widget_control_styles_list($widget_name, $id, $value2, $value3) {
	$themename = 'mymagpie';

	echo '<div>';
	// prepare an array of options
	$items = array('<option value="" selected="selected">'.__('None', MYMAGPIE_PLUGIN_DOMAIN).'</option>');

	// prepare the responsive select
	$items = array(
		'<option value="visible-all-devices"'.((!$value2 || $value2 == 'visible-all-devices') ? ' selected="selected"' : '').'>'.__('All devices', MYMAGPIE_PLUGIN_DOMAIN).'</option>',
		'<option value="visible-desktop"'.(($value2 == 'visible-desktop') ? ' selected="selected"' : '').'>'.__('Desktops', MYMAGPIE_PLUGIN_DOMAIN).'</option>',
		'<option value="visible-tablet"'.(($value2 == 'visible-tablet') ? ' selected="selected"' : '').'>'.__('Tablets', MYMAGPIE_PLUGIN_DOMAIN).'</option>',
		'<option value="visible-phone"'.(($value2 == 'visible-phone') ? ' selected="selected"' : '').'>'.__('Phones', MYMAGPIE_PLUGIN_DOMAIN).'</option>',
		'<option value="hidden-phone"'.(($value2 == 'hidden-phone') ? ' selected="selected"' : '').'>'.__('Desktops/Tablets', MYMAGPIE_PLUGIN_DOMAIN).'</option>',
		'<option value="hidden-desktop"'.(($value2 == 'hidden-desktop') ? ' selected="selected"' : '').'>'.__('Tablets/Phones', MYMAGPIE_PLUGIN_DOMAIN).'</option>',
		'<option value="hidden-tablet"'.(($value2 == 'hidden-tablet') ? ' selected="selected"' : '').'>'.__('Desktops/Phones', MYMAGPIE_PLUGIN_DOMAIN).'</option>'
	);
	// output the responsive select
	echo '<p class="device_visible"><label for="' . $themename . '_widget_responsive_'.$id.'">'.__('Visible on', MYMAGPIE_PLUGIN_DOMAIN).': <select name="' . $themename . '_widget_responsive_'.$id.'"  id="' . $themename . '_widget_responsive_'.$id.'" class="widefat">';
	//
	...
	$items = array(
		'<option value="all"'.(($value3 == null || !$value3 || $value3 == 'all') ? ' selected="selected"' : '').'>'.__('All users', MYMAGPIE_PLUGIN_DOMAIN).'</option>',
		'<option value="guests"'.(($value3 == 'guests') ? ' selected="selected"' : '').'>'.__('Only guests', MYMAGPIE_PLUGIN_DOMAIN).'</option>',
		'<option value="registered"'.(($value3 == 'registered') ? ' selected="selected"' : '').'>'.__('Only registered users', MYMAGPIE_PLUGIN_DOMAIN).'</option>',
		'<option value="administrator"'.(($value3 == 'administrator') ? ' selected="selected"' : '').'>'.__('Only administrator', MYMAGPIE_PLUGIN_DOMAIN).'</option>'
	);
	//
	echo '<p class="users_visible"><label for="' . $themename . '_widget_users_'.$id.'">'.__('Visible for', MYMAGPIE_PLUGIN_DOMAIN).': <select name="' . $themename . '_widget_users_'.$id.'"  id="' . $themename . '_widget_users_'.$id.'" class="widefat">';
	//
	foreach($items as $item)
		echo $item;
	//
...
function check_widget_visibility($id) {
	...
	if ( isset($options[$id]) && $options[$id] != '' ) {
		// create function
		$conditional_function = create_function('', 'return '.mymagpie_condition($options_type[$id], $options[$id], $users[$id]).';');
	...
	$wp_registered_widgets[$id]['mymagpiestate'] = $conditional_result;
 ...
 *
 **/
function mymagpie_condition($mode, $input, $users) {
	// Example input:
