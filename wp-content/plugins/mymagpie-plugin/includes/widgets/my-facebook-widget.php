<?php
	class My_Facebook_Widget extends WP_Widget {
		public function __construct() {
			parent::__construct('My_Facebook_Widget', __('Mymagpie - Facebook Like Box Widget', MYMAGPIE_PLUGIN_DOMAIN), array( 'description' => __('Mymagpie - Facebook Like Box Widget', MYMAGPIE_PLUGIN_DOMAIN)) );
		}
	
		public function widget( $args, $instance ){
			extract($args, EXTR_SKIP);
			$title          = apply_filters('widget_title', empty($instance['title']) ? __('My Facebook Page', MYMAGPIE_PLUGIN_DOMAIN) : $instance['title']);
		...
		public function form( $instance ){   
			...
			<!--facebook_URL-->
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('facebook_URL')); ?>"><?php _e('Facebook page url', MYMAGPIE_PLUGIN_DOMAIN).':'; ?></label>
				...
				<span style="font-size:11px; color:#999;"><?php _e('The Like Box only works with ', MYMAGPIE_PLUGIN_DOMAIN); ?><a target="_blank" href="https://www.facebook.com/help/174987089221178/" title="<?php _e('Facebook Pages.', MYMAGPIE_PLUGIN_DOMAIN); ?>"><?php _e('Facebook Pages.', MYMAGPIE_PLUGIN_DOMAIN); ?></a></span>
			</p>
			<!--box_width-->
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('box_width')); ?>"><?php _e('Width', MYMAGPIE_PLUGIN_DOMAIN); ?></label>
			...
			<!--box_height-->
			<p>
				<label for="<?php echo esc_attr($this->get_field_id('box_height')); ?>"><?php _e('Height', MYMAGPIE_PLUGIN_DOMAIN); ?></label>
			...
			<!--color_scheme-->
			<p>
				<label for="<?php echo $this->get_field_id('color_scheme'); ?>"><?php _e('Color Scheme', MYMAGPIE_PLUGIN_DOMAIN).":"; ?> 
					<select id="<?php echo $this->get_field_id('color_scheme'); ?>" name="<?php echo $this->get_field_name('color_scheme'); ?>" style="width:140px;" > 
						<option value="light" <?php echo ($color_scheme === 'light' ? ' selected="selected"' : ''); ?>><?php _e('Light', MYMAGPIE_PLUGIN_DOMAIN) ?></option>
						<option value="dark" <?php echo ($color_scheme === 'dark' ? ' selected="selected"' : ''); ?>><?php _e('Dark', MYMAGPIE_PLUGIN_DOMAIN) ?></option>
					</select>
				</label>
			</p>
			<!--display_haeder-->
			<p>
				<input class="checkbox" id="<?php echo $this->get_field_id('display_haeder'); ?>" name="<?php echo $this->get_field_name('display_haeder'); ?>" type="checkbox" <?php checked($instance['display_haeder'], 'on' ); ?> /> <label for="<?php echo $this->get_field_id('display_haeder'); ?>"><?php _e('Display header', MYMAGPIE_PLUGIN_DOMAIN)."."; ?></label>
			</p>
			<!--display_faces-->
			<p>
				<input class="checkbox" id="<?php echo $this->get_field_id('display_faces'); ?>" name="<?php echo $this->get_field_name('display_faces'); ?>" type="checkbox" <?php checked($instance['display_faces'], 'on' ); ?> /> <label for="<?php echo $this->get_field_id('display_faces'); ?>"><?php _e('Display faces', MYMAGPIE_PLUGIN_DOMAIN)."."; ?></label>
			</p>
			<!--display_stream-->
			<p>
				<input class="checkbox" id="<?php echo $this->get_field_id('display_stream'); ?>" name="<?php echo $this->get_field_name('display_stream'); ?>" type="checkbox" <?php checked($instance['display_stream'], 'on' ); ?> /> <label for="<?php echo $this->get_field_id('display_stream'); ?>"><?php _e('Display stream', MYMAGPIE_PLUGIN_DOMAIN)."."; ?></label>
			</p>
			<!--display_border-->
			<p>
				<input class="checkbox" id="<?php echo $this->get_field_id('display_border'); ?>" name="<?php echo $this->get_field_name('display_border'); ?>" type="checkbox" <?php checked($instance['display_border'], 'on' ); ?> /> <label for="<?php echo $this->get_field_id('display_border'); ?>"><?php _e('Display border', MYMAGPIE_PLUGIN_DOMAIN)."."; ?></label>
			</p>
			<?php
		}
	}
