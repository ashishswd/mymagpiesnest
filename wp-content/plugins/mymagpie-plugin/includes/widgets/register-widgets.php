 ...
 *
 */
function mymagpie_load_widgets() {
	$widget_files = array(
		...
		'Mymagpie_Instagram_Widget' => 'mymagpie-instagram-widget.php',
		'Mymagpie_Banner_Widget'    => 'mymagpie-banner-widget.php',
	);
	foreach ( $widget_files as $class_name => $file_name ) {
		$widget_dir = file_exists( CURRENT_THEME_DIR . '/includes/widgets/' . $file_name ) ? CURRENT_THEME_DIR . '/includes/widgets/' . $file_name : MYMAGPIE_PLUGIN_DIR . 'includes/widgets/' . $file_name ;
...
add_action( 'widgets_init', 'mymagpie_load_widgets' );
?>