...
<script>
	jQuery(document).ready(function() {
		...
		function set_date(){
			...
			if(left_time<0){
				jQuery('#under_construction_timer').html("<span class='web_site_message'><?php _e('Website is currently down for maintenance.', MYMAGPIE_PLUGIN_DOMAIN); ?></span>");
			}
		}
...
<?php 
	...
	<?php if($get_theme['Template']=='MymagpieFramework'){
...
if(isset($mtc_options['mtc_mode_timer'])){ ?>
	<div id="under_construction_timer">
		<div id="days_left" class="box">
			...
			<div class="lable"><?php _e('Days', MYMAGPIE_PLUGIN_DOMAIN); ?></div>
		</div>
		<div id="hour_left" class="box">
			<div class="numbers"></div>
			<div class="lable"><?php _e('Hours', MYMAGPIE_PLUGIN_DOMAIN); ?></div>
		</div>
		<div id="minute_left" class="box">
			<div class="numbers"></div>
			<div class="lable"><?php _e('Minuts', MYMAGPIE_PLUGIN_DOMAIN); ?></div>
		</div>
		<div id="seconds_left" class="box">
			<div class="numbers"></div>
			<div class="lable"><?php _e('Seconds', MYMAGPIE_PLUGIN_DOMAIN); ?></div>
		</div>
	</div>
<?php 
};
