<?php
	...
	add_action('wp_ajax_export_content', 'mymagpie_plugin_export_content');
	function mymagpie_plugin_export_content() {
		...
		$response = array(
			...
			'id'=>'1',
			'data'=>__('Export content done', MYMAGPIE_PLUGIN_DOMAIN),
		);
		$response_file = array(
			'what'=>'file',
			'action'=>'export_content',
		...
		mymagpie_plugin_delete_file($zip_name);

		if(is_dir(UPLOAD_BASE_DIR)){
			$file_string = mymagpie_plugin_scan_dir(UPLOAD_BASE_DIR, $exclude_folder, $exclude_files);
		}
	...
	function mymagpie_plugin_export_xml(){
		ob_start();
		export_wp();
		$xml = ob_get_clean();
	...
	function mymagpie_plugin_export_json() {
		$themename = 'mymagpie';
	...
	function mymagpie_plugin_delete_file($file){
		if(is_readable($file)){
			unlink($file);
			return 'file deleted';
		}else{
	...
	function mymagpie_plugin_scan_dir($dir, $exceptions_folder, $exceptions_files){
		...
		foreach ($scand_dir as $file) {
			...
			if(is_dir($scan_file)){
				$scan_file= mymagpie_plugin_scan_dir($scan_file, $exceptions_folder, $exceptions_files);
