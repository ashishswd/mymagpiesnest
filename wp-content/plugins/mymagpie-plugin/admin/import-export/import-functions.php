<?php
	add_action('wp_ajax_import_json', 'mymagpie_plugin_import_json');
	function mymagpie_plugin_import_json() {
		do_action( 'mymagpie_plugin_import_json' );
		...
		if( mymagpie_plugin_parse_import_data( $sidebar_data ) ) {
			exit('import_end');
		}else{
			exit('error');
		};
	}
	function mymagpie_plugin_parse_import_data( $import_array ) {
	...
	add_action('wp_ajax_import_xml', 'mymagpie_plugin_import_xml');
	function mymagpie_plugin_import_xml(){
		$nonce = $_POST['nonce'];
		...
		do_action( 'mymagpie_plugin_start_import' );

		$_SESSION = array();
		do_action( 'mymagpie_plugin_import_xml' );
	
		add_filter( 'import_post_meta_key', 'mymagpie_plugin_is_valid_meta_key' );

		$xml_file = isset($_POST['file']) ? $_POST['file'] : 'false' ;
		$import_data = mymagpie_plugin_parse_xml( UPLOAD_DIR.$xml_file );
		...
		mymagpie_plugin_import_start();

		exit('import_categories');
	}
	function mymagpie_plugin_import_start(){
		wp_defer_term_counting( true );
		wp_defer_comment_counting( true );
		wp_suspend_cache_invalidation( true );
	}
	function mymagpie_plugin_import_end(){
		wp_cache_flush();
		...
		update_option('mymagpie_sample_data', 1);

		mymagpie_plugin_set_to_draft('hello-world');
		mymagpie_plugin_set_to_draft('sample-page');

		settings();

		do_action( 'mymagpie_plugin_import_end' );
		session_name("import_xml");
		session_destroy();
		exit('import_json');
	}
	add_action('wp_ajax_import_categories', 'mymagpie_plugin_import_categories');
	function mymagpie_plugin_import_categories() {
		$nonce = $_POST['nonce'];
		if ( !wp_verify_nonce( $nonce, 'import_ajax-nonce' ) )
			exit ( 'instal_error');
		...
		do_action( 'mymagpie_plugin_import_categories' );
	...
	add_action('wp_ajax_import_tags', 'mymagpie_plugin_import_tags');
	function mymagpie_plugin_import_tags() {
		$nonce = $_POST['nonce'];
		if ( !wp_verify_nonce( $nonce, 'import_ajax-nonce' ) )
			exit ( 'instal_error');
		...
		do_action( 'mymagpie_plugin_import_tags' );
	...
	add_action('wp_ajax_process_terms', 'mymagpie_plugin_process_terms');
	function mymagpie_plugin_process_terms() {
		$nonce = $_POST['nonce'];
		if ( !wp_verify_nonce( $nonce, 'import_ajax-nonce' ) )
			exit ( 'instal_error');
		...
		do_action( 'mymagpie_plugin_process_terms' );
	...
	add_action('wp_ajax_import_posts', 'mymagpie_plugin_import_posts');
	function mymagpie_plugin_import_posts() {
		$nonce = $_POST['nonce'];
		if ( !wp_verify_nonce( $nonce, 'import_ajax-nonce' ) )
			exit ( 'instal_error');
		...
		do_action( 'mymagpie_plugin_import_posts' );
					...
					if ( ! empty( $post['terms'] ) ) {
						...
						foreach ( $post['terms'] as $term ) {
							...
							if ( ! $term_id ) {
								...
								if ( ! is_wp_error( $t ) ) {
									$term_id = $t['term_id'];
									do_action( 'mymagpie_plugin_import_insert_term', $t, $term, $post_id, $post );
								} else {
									// Failed to import
									do_action( 'mymagpie_plugin_import_insert_term_failed', $t, $term, $post_id, $post );
									continue;
								}
							}
					...
					if ( ! empty( $post['comments'] ) ) {
						...
						foreach ( $newcomments as $key => $comment ) {
							...
							if ( ! $post_exists || ! comment_exists( $comment['comment_author'], $comment['comment_date'] ) ) {
								...
								do_action( 'mymagpie_plugin_import_insert_comment', $inserted_comments[$key], $comment, $comment_post_ID, $post );
					...
					if ( isset( $post['postmeta'] ) ) {
						foreach ( $post['postmeta'] as $meta ) {
							...
							if ( $key ) {
								...
								do_action( 'mymagpie_plugin_import_post_meta', $post_id, $key, $value );
	...
	add_action('wp_ajax_import_menu_item', 'mymagpie_plugin_import_menu_item');
	function mymagpie_plugin_import_menu_item() {
		$nonce = $_POST['nonce'];
		if ( !wp_verify_nonce( $nonce, 'import_ajax-nonce' ) )
			exit ( 'instal_error');
		...
		do_action( 'mymagpie_plugin_import_menu_item' );
	...
	function mymagpie_plugin_add_item( $item ) {
	...
	add_action('wp_ajax_import_attachment', 'mymagpie_plugin_import_attachment');
	function mymagpie_plugin_import_attachment() {
		$nonce = $_POST['nonce'];
		if ( !wp_verify_nonce( $nonce, 'import_ajax-nonce' ) )
			exit ( 'instal_error');
		...
		if(!empty($_SESSION['attachment_posts'])){
			do_action( 'mymagpie_plugin_import_attachment' );
			...
			foreach ( $posts_array as $post ) {
				...
				if(file_exists($file_url)){
					mymagpie_plugin_add_attachment( $postdata, $remote_url );
				}
			}
		}
	...
	function mymagpie_plugin_add_attachment( $post, $url ) {
	...
	add_action('wp_ajax_generate_attachment_metadata', 'mymagpie_plugin_generate_attachment_metadata');
	function mymagpie_plugin_generate_attachment_metadata() {
		$nonce = $_POST['nonce'];
		if ( !wp_verify_nonce( $nonce, 'import_ajax-nonce' ) )
			exit ( 'instal_error');
		...
		do_action( 'mymagpie_plugin_generate_attachment_metadata' );
		if(!empty($_SESSION['attachment_posts'])){
	...
	add_action('wp_ajax_import_attachment_metadata', 'mymagpie_plugin_import_attachment_metadata');
	function mymagpie_plugin_import_attachment_metadata() {
		$nonce = $_POST['nonce'];
		if ( !wp_verify_nonce( $nonce, 'import_ajax-nonce' ) )
			exit ( 'instal_error');
	...
	add_action('wp_ajax_import_parents', 'mymagpie_plugin_import_parents');
	function mymagpie_plugin_import_parents() {
		$nonce = $_POST['nonce'];
		if ( !wp_verify_nonce( $nonce, 'import_ajax-nonce' ) )
			exit ( 'instal_error');
		...
		do_action( 'mymagpie_plugin_import_parents' );
	...
	add_action('wp_ajax_update_attachment', 'mymagpie_plugin_update_attachment');
	function mymagpie_plugin_update_attachment() {
		$nonce = $_POST['nonce'];
		if ( !wp_verify_nonce( $nonce, 'import_ajax-nonce' ) )
			exit ( 'instal_error');
		...
		do_action( 'mymagpie_plugin_update_attachment' );
		...
		mymagpie_plugin_import_end();
	}
	function sort_array( $a, $b ) {
		return strlen($b) - strlen($a);
	}
	add_action('wp_ajax_update_featured_images', 'mymagpie_plugin_update_featured_images');
	function mymagpie_plugin_update_featured_images() {
		$nonce = $_POST['nonce'];
		if ( !wp_verify_nonce( $nonce, 'import_ajax-nonce' ) )
			exit ( 'instal_error');
		...
		do_action( 'mymagpie_plugin_update_featured_images' );
	...
	function mymagpie_plugin_parse_xml( $file ) {
	...
	function settings() {
		...
		do_action( 'mymagpie_plugin_set_settings' );
	...
	*/
	function mymagpie_plugin_set_to_draft($title) {
		global $wpdb;
	...
	function mymagpie_plugin_is_valid_meta_key( $key ) {
...
//remove inactive widgets
	function mymagpie_plugin_remove_inactive_widgets(){
		$widgets = get_option( 'sidebars_widgets' );
		$widgets['wp_inactive_widgets'] = array();
		update_option( 'sidebars_widgets', $widgets );
		exit('done');
	}
	add_action('wp_ajax_remove_widgets', 'mymagpie_plugin_remove_inactive_widgets');
?>