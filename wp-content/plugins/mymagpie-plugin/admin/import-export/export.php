			...
			jQuery.post(ajaxurl, {action: 'export_content'}, function(response) {
				...
				if(!res.errors){
					...
					if(file.indexOf('.zip')!=-1){
						window.location.href = '<?php echo (MYMAGPIE_PLUGIN_URL."admin/import-export/download-content.php"); ?>?file=' + file;
					}
				}else{
					//console.log('error 1');
				}
...
<p><?php _e("Export allows you to create a backup of your website content in one click. You'll get a downloaded archive containing all website data: images, audio, video and other files from your media library. XML file with your posts and categories data, JSON file with widget settings. You can use downloaded archive to move your website to other hosting server or restore website data. ", MYMAGPIE_PLUGIN_DOMAIN); ?></p>
<p><b><?php _e("Please note! ", MYMAGPIE_PLUGIN_DOMAIN);  ?></b><?php _e("XML file doesn't contain any user data except name and email of the website administrator.", MYMAGPIE_PLUGIN_DOMAIN); ?></p>
<a id="export-content" class="button button-primary float-left" href="#"><?php _e('Export Content', MYMAGPIE_PLUGIN_DOMAIN); ?></a><span class="spinner"></span>