<?php
	...
	if($browser_not_supported){
		...
		do_action( 'mymagpie_plugin_fail_server_settings' );
		echo '<h3>Caution!</h3>';
		echo "<h4 class='title'>" . __('Some of your server settings do not meet the requirements for installing the sample data. Please, consult with your hosting provider on how to increase the required values.', MYMAGPIE_PLUGIN_DOMAIN) . "</h4>";
		echo "<table width='100%' border='0' cellspacing='0' cellpadding='4' style='margin-bottom: 15px;' class='wp-list-table widefat fixed'>";
		echo "<thead><tr>";
		echo "<th>" . __('Server Settings', MYMAGPIE_PLUGIN_DOMAIN) . "</th>";
		echo "<th>" . __('Current', MYMAGPIE_PLUGIN_DOMAIN) . "</th>";
		echo "<th>" . __('Required', MYMAGPIE_PLUGIN_DOMAIN) . "</th>";
		echo "</tr></thead>";
		echo "<tbody>";
		$count = 0;
		...
		echo "</tbody>";
		echo "</table>";
		echo '<a class="button button-primary" href="admin.php?page=import-page&amp;import=true">'.__('Continue', MYMAGPIE_PLUGIN_DOMAIN).'</a>';
		$_SESSION['import']=true;
	}else{
		include_once (MYMAGPIE_PLUGIN_DIR.'admin/import-export/import.php');
	}
	do_action( 'mymagpie_plugin_server_settings' );
