<?php
class mymagpie_plugin_components {
	public function get_header($attr){
		...
		$get_remote_info = mymagpie_plugin_remote_query(array('data_type' => 'info'));
		...
		$output = '<div class="wrap mymagpie-plugin">';
		$output .= '<header><h2>';
		$output .= ($title) ? $title : '' ;
		$output .= '<span class="extern-links">';
		$output .= '<a href="'.$doc_link.'" target="_blank"><span class="icon-book"></span>'.__('Documentation', MYMAGPIE_PLUGIN_DOMAIN).'</a>';
		$output .= '<a href="'.$support_link.'" target="_blank"><span class="icon-wrench"></span>'.__('Support', MYMAGPIE_PLUGIN_DOMAIN).'</a>';
		$output .= '</span></h2>';
