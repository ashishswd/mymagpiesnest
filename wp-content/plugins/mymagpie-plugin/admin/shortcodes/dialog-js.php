...
?>
var shortcode_generator_url = "<?php echo esc_url( MYMAGPIE_PLUGIN_URL); ?>" + "admin/shortcodes/",
	mymagpieSelectedShortcodeType,
	tb_dialog_helper = {
		mymagpieSelectedShortcodeType: '',
		needsPreview: false,
		...
		setupShortcodeType: function ( shortcode ) {
			mymagpieSelectedShortcodeType = shortcode;
			this.mymagpieSelectedShortcodeType = shortcode;
		},

		loadShortcodeDetails: function() {
			if (mymagpieSelectedShortcodeType) {
				var a = this;
				// Clean out the table rows before applying the new ones.
				jQuery( '#options-table' ).html( '' );
				jQuery.getScript(shortcode_generator_url + "shortcodes/my_" + mymagpieSelectedShortcodeType + ".js", function () {
					a.initializeDialog();
		...
		initializeDialog: function (){
			if (typeof frameworkShortcodeAtts == "undefined") {
				jQuery("#shortcode-options").append("<p>Error loading details for shortcode: " + mymagpieSelectedShortcodeType + "</p>");
			} else {
