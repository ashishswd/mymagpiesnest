(function(){
	...
	if ( scripts.length ) {
		...
		for ( i in scripts ) {
			...
			var txt = scriptSrc.search( 'mymagpie-plugin' );
	...
	tinymce.create(
		"tinymce.plugins.MymagpieTinyMCEShortcodes",
		{
			init: function(d,e) {
					var nonce = '';
					if ( nonce == '' ) {
						jQuery.post( ajaxurl, { 'action' : 'mymagpie_shortcodes_nonce' }, function ( response ) {
							nonce = response;
						});
					}
					...
					d.addCommand( "myThemeOpenDialog",function(a,c){
						...
						mymagpieSelectedShortcodeType = c.identifier;
						mymagpieSelectedShortcodeTitle = c.title;

						// jQuery.get(e+"/dialog.php",function(b){

							jQuery('#shortcode-options').addClass( 'shortcode-' + mymagpieSelectedShortcodeType );
							jQuery( '#selected-shortcode' ).val( mymagpieSelectedShortcodeType );

							// Skip the popup on certain shortcodes.

							switch ( mymagpieSelectedShortcodeType ) {
				...
				// Accordion
					...
					// small
								...
								tb_dialog_helper.setupShortcodeType( mymagpieSelectedShortcodeType );

								tb_show("Insert "+ mymagpieSelectedShortcodeTitle +" Shortcode", "#TB_inline?width="+f+"&height="+b+"&inlineId=dialog");jQuery("#shortcode-options h3:first").text(""+c.title+" Shortcode Settings");
				...
				createControl:function(d,e){
						...
						if(d=="mymagpie_shortcodes_button"){

							d=e.createMenuButton("mymagpie_shortcodes_button",{
		...
		tinymce.PluginManager.add("MymagpieTinyMCEShortcodes",tinymce.plugins.MymagpieTinyMCEShortcodes)
	}
)();