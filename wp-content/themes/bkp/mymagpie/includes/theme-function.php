 ...
 */
function mymagpie_show_portfolio_filter() {
...
add_action('restrict_manage_posts', 'mymagpie_show_portfolio_filter');

/**
 * Filter portfolio by cats and tags
 */
function mymagpie_portfolio_filter_query( $query ) {
	global $typenow, $wp_query;
...
add_filter( 'parse_query', 'mymagpie_portfolio_filter_query' );
...
if ( !function_exists( 'tz_grid_gallery' ) ) {
	...
	function tz_grid_gallery($postid, $imagesize) {
		...
		if ( $single_gallery_layout == 'masonry' ) {
			...
			add_action( 'wp_footer', 'mymagpie_enqueue_isotope' );
...
function mymagpie_enqueue_isotope() {
