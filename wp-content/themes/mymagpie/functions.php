<?php
	...
	@define( 'MYMAGPIE_VER', mymagpie_get_theme_version('MymagpieFramework') );
	...
	*
	**/
	function mymagpie_js_global_variables(){
		$output = "<script>";
	...
	add_action('wp_head', 'mymagpie_js_global_variables');
	add_action('admin_head', 'mymagpie_js_global_variables');
	...
	*
	**/
	function mymagpie_get_theme_version($theme_name) {
		if ( function_exists('wp_get_theme') ) {
	...
	*
	**/
	if ( CURRENT_THEME != 'mymagpie' )
		add_action('mymagpie_activation_hook', 'comment_child_var');
	...
	*
	**/
	add_action('mymagpie_activation_hook', 'clean_less_cache');
	...
	if ( (is_admin() && ($pagenow == "themes.php")) && FILE_WRITEABLE ) {
		do_action('mymagpie_activation_hook');
	}

	if ( !function_exists('mymagpie_theme_setup')) {
		function mymagpie_theme_setup() {
		...
		add_action('after_setup_theme', 'mymagpie_theme_setup');
	}
	...
	function mymagpie_wpml_translate_filter( $value, $name ) {
		return icl_translate( 'mymagpie', $name, $value );
	}
	//Check if WPML is activated
	if ( defined( 'ICL_SITEPRESS_VERSION' ) ) {
		add_filter( 'mymagpie_text_translate', 'mymagpie_wpml_translate_filter', 10, 2 );
	}
		...
		function myHelpPointers() {
			...
			$pointers = array(
				...
				array(
					...
					'target'   => '#toplevel_page_mymagpie-plugin-page', // the css selector for the pointer to be tied to, best to use ID's
					'title'    => theme_locals("import_sample_data"),
					'content'  => theme_locals("import_sample_data_desc"),
					'position' => array(
										'edge'   => 'left', //top, bottom, left, right
	...
	if (is_user_logged_in() && current_user_can('update_themes')) {
		...
		$oldMymagpieVersion = get_option('mymagpie_version', '1.0');
		$currentMymagpieVersion = getMymagpieVersion($updateErrors);
		$mymagpieV2 = '2.0';
		$mymagpieForceUpdate = (bool) get_option('mymagpie_force_update', false);

		if (version_compare($oldMymagpieVersion, $mymagpieV2) == -1 || $mymagpieForceUpdate) {
			writeLog(PHP_EOL . date('Y-m-d H:i:s'));
			writeLog('Old MymagpieFramework version: ' . $oldMymagpieVersion);
			writeLog('Current MymagpieFramework version: ' . $currentMymagpieVersion);
			writeLog('Force update: ' . (int) $mymagpieForceUpdate);

			mymagpieUpdate($mymagpieTemplates, $headerFooterPattern, $updateErrors);
			//changeChildTheme
			changeChildTheme($updateErrors);
			//end changeChildTheme
			if (empty($updateErrors)) {
				update_option('mymagpie_version', $currentMymagpieVersion);
				update_option('mymagpie_force_update', 0);
			}
		}
	}

	function getMymagpieVersion(&$updateErrors) {
		$style = PARENT_DIR . '/style.css';
		$themeVersion = 0;

		if (function_exists('wp_get_theme')) {
			$theme = wp_get_theme(get_option('template', 'MymagpieFramework'));
	...
	function isMymagpieChildTheme($themePath, $name, &$updateErrors) {
		...
		return ($themeTemplate == 'MymagpieFramework') ? true : false;
	}
	...
	function mymagpieUpdate($mymagpieTemplates, $headerFooterPattern, &$updateErrors) {
		$themesPath = get_theme_root();

		$skip = array('.', '..', 'MymagpieFramework', 'twentytwelve', 'twentyeleven', 'twentyten', 'index.php');
		...
		if (!empty($themes)) {
			foreach ($themes as $theme) {
				...
				if (is_dir($themePath) && file_exists($themePath . '/style.css')) {
					...
					$isMymagpieChildTheme = isMymagpieChildTheme($themePath, $theme, $updateErrors);

					if ($isMymagpieChildTheme) {
						if (is_writable($themePath)) {
							writeLog(PHP_EOL . 'Child theme: ' . $themePath);
							$files = scandir($themePath);
							$themeTemplates = array_intersect($files, $mymagpieTemplates);
...
//end changeChildTheme
	if (has_action('after_switch_theme')) {
		add_action('after_switch_theme', 'activateMymagpieForceUpdate', 10, 0);
	} else {
		if (is_admin() && isset($_GET['activated']) && $pagenow == 'themes.php') {
			activateMymagpieForceUpdate();
		}
	}

	function activateMymagpieForceUpdate() {
		global $mymagpieTemplates;
		...
		if ($themeTemplate == 'MymagpieFramework') {
			$themePath = get_stylesheet_directory();
			$files = scandir($themePath);
			$themeTemplates = array_intersect($files, $mymagpieTemplates);

			$mymagpieForceUpdate = false;
			...
			if ($mymagpieForceUpdate) {
				update_option('mymagpie_force_update', 1);
			} else {
				update_option('mymagpie_force_update', 0);
			}
		}
...
//------------------------------------------------------
	...
	function get_post_metadata( $args = array() ) {
		...
		if($post_meta_type!='false' && $args['display_meta_data']){
			...
			$get_voting_array = mymagpie_getPostVoting($post_ID, $user_id);
						...
						case 'views':
							if(of_get_option('post_views') != 'no'){ ?>
								<div class="post_views" title="<?php echo theme_locals('number_views'); ?>">
									...
									<?php echo $icon_tips_before.mymagpie_getPostViews($post_ID).$icon_tips_after; ?>
								</div>
								<?php
							}
							break;
...
//  Post voting
//------------------------------------------------------
	function mymagpie_getPostVoting($postID, $user_id){
...
//------------------------------------------------------
	function mymagpie_get_post_networks($args = array()){
		global $post;
		extract(
			wp_parse_args(
				$args,
...
//  Related Posts
//------------------------------------------------------
	if(!function_exists('mymagpie_related_posts')){
		function mymagpie_related_posts($args = array()){
			global $post;
			$default = array(
			...
			$blog_related = apply_filters( 'mymagpie_text_translate', of_get_option('blog_related'), 'blog_related' );
...
//------------------------------------------------------
	if (of_get_option('main_layout') == 'fixed') {
		add_filter('body_class','mymagpie_layout_class');
		function mymagpie_layout_class($classes) {
			$classes[] = 'mymagpie-fixed-layout';
...
//------------------------------------------------------
	if ( (of_get_option('header_background') != '')
		...
		add_action('wp_head', 'mymagpie_general_opt');
		function mymagpie_general_opt(){
			$output = "\n<style type='text/css'>";
	...
	*
	**/
	add_action('load-update.php', 'mymagpie_register_update_page_hook');
	function mymagpie_register_update_page_hook(){
		if ( isset($_GET['action']) ) {
			$plugin = isset($_REQUEST['plugin']) ? trim($_REQUEST['plugin']) : '';
			$theme  = isset($_REQUEST['theme']) ? urldecode($_REQUEST['theme']) : '';
			$action = isset($_REQUEST['action']) ? $_REQUEST['action'] : '';

			if ( 'MymagpieFramework' == $theme ) {

				if ( 'upgrade-theme' == $action ) {
					add_action('upgrader_process_complete', 'after_mymagpie_theme_upgrade_call');
				} elseif ( 'upload-theme' == $action ) {
					add_action('upgrader_process_complete', 'after_mymagpie_theme_upgrade_call');
				}
			}

			if ( 'mymagpie-plugin' == $plugin ) {

				if ( 'upgrade-plugin' == $action ) {
					add_action('upgrader_process_complete', 'after_mymagpie_plugin_upgrade_call');
				}
			}
		}
	}
	...
	*
	**/
	add_action('load-update-core.php', 'mymagpie_register_update_core_page_hook');
	function mymagpie_register_update_core_page_hook(){
		...
		if ( array_search('MymagpieFramework', $themes) === FALSE )
			return;
	...
	/**
	*
	* Register hook after MymagpieFramework upgrade
	*
	**/
	function after_mymagpie_theme_upgrade_call(){
		do_action('after_mymagpie_theme_upgrade');
	}

	/**
	*
	* Register hook after Mymagpie Plugin upgrade
	*
	**/
	function after_mymagpie_plugin_upgrade_call(){
		do_action('after_mymagpie_plugin_upgrade');
	}

	/**
	*
	* Unpack Mymagpie Plugin package
	*
	**/
	add_action('after_mymagpie_theme_upgrade', 'mymagpie_plugin_unpack_package');
	function mymagpie_plugin_unpack_package(){
		$plugin = 'mymagpie-plugin/mymagpie-plugin.php';
		...
		$file   = PARENT_DIR . '/includes/plugins/mymagpie-plugin.zip';
		$to     = WP_PLUGIN_DIR . '/mymagpie-plugin/';
		$result = false;
	...
	/**
	*
	* Set Up Mymagpie Plugin
	*
	**/
	add_action('after_setup_theme', 'mymagpie_plugin_setup');
	function mymagpie_plugin_setup(){
		$plugin = 'mymagpie-plugin/mymagpie-plugin.php';
	...
	*
	**/
	if ( !function_exists('mymagpie_get_layout_class') ) {
		function mymagpie_get_layout_class($layout) {
			switch ($layout) {

				case 'full_width_content':
					$layout_class = apply_filters( "mymagpie_layout_wrapper", "span12" );
					break;

				case 'content':
					$layout_class = apply_filters( "mymagpie_layout_content_column", "span8" );
					$layout_class .= ' '.of_get_option('blog_sidebar_pos');
					break;

				case 'sidebar':
					$layout_class = apply_filters( "mymagpie_layout_sidebar_column", "span4" );
					break;

				case 'left_block':
					$layout_class = apply_filters( "mymagpie_layout_left_block_column", "span7" );
					break;

				case 'right_block':
					$layout_class = apply_filters( "mymagpie_layout_right_block_column", "span5" );
					break;
			}
