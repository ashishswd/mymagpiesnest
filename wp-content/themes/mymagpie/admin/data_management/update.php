...
/******************Change this*******************/
@define('API_URL', 'http://updates.mymagpie.template-help.com/mymagpiemoto/v3/api/');
/************************************************/
...
function check_for_update($checked_data) {
	...
	if (!empty($response)){
		...
		update_option('mymagpie_new_version', $response["new_version"]);
		update_option('mymagpie_url_info', $response["url"]);
	}
	return $checked_data;
}
...
function wp_persistant_notice() {
	global $pagenow;
	$mymagpie_url_info = get_option('mymagpie_url_info');
	$mymagpie_new_version = get_option('mymagpie_new_version');
	$mymagpie_version = get_theme_info(PARENT_NAME, 'Version');
	$pageHidden = array("update.php", "update-core.php", 'mymagpie-options_page_options-framework-data-management', 'admin.php');
	$update_url = wp_nonce_url('update.php?action=upgrade-theme&amp;theme=' . urlencode(PARENT_NAME), 'upgrade-theme_'.urlencode(PARENT_NAME));

	if (! get_user_meta(get_current_user_id(), '_wp_hide_notice', true) &&  !in_array($pagenow, $pageHidden) && is_admin() && ($mymagpie_new_version > $mymagpie_version ) ) {
		printf( '<div class="updated"><p><strong>%1$s <a href="%2$s" class="thickbox" title="mymagpie">%3$s</a> %4$s <a href="%5$s" onclick="%6$s">%7$s</a><br>%8$s<br><a href="%9$s"> %10$s </a></strong></p></div>', theme_locals('new_version'), $mymagpie_url_info.'?TB_iframe=true&width=1024&height=800', theme_locals('view_version').' '.$mymagpie_new_version.' '.theme_locals('details'), theme_locals('or'), $update_url, "if ( confirm('Updating this theme will lose any customizations you have made. \'Cancel\' to stop, \'OK\' to update.') ) {return true;}return false;", theme_locals('update_now'), theme_locals('info_box_4'), esc_url(add_query_arg( 'wp_nag', wp_create_nonce( 'wp_nag' ))), theme_locals('dismiss_notice'));
	}
}
